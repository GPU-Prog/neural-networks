...
void forward(ann_t *nn, double (*activation_function)(double))
{
    for (int l = 1; l < nn->number_of_layers; l++)
    {
        ...
        matrix_dot_GPU_shared<<<gridDim, blockDim>>>(nn->layers[l]->weights, nn->layers[l-1]->activations, z1);
        ...
        matrix_dot_GPU_shared<<<gridDim2, blockDim2>>>(nn->layers[l]->biases, one, z2);
...
void backward(ann_t *nn, matrix_t *y, double (*derivative_actfunct)(double))
{
    ...
    for (int l = L; l > 1; l--)
    {
        ...
        matrix_dot_GPU_shared<<<gridDim_dot, blockDim_dot>>>(tw, nn->layers[l]->delta, delta_tmp);
    ...
    for (int l = 1; l < nn->number_of_layers; l++)
    {
        ...
        matrix_dot_GPU_shared<<<gridDim_dot2, blockDim_dot2>>>(nn->layers[l]->delta, ta, w1);
        ...
        matrix_dot_GPU_shared<<<gridDim_dot3, blockDim_dot3>>>(nn->layers[l]->delta, one, b1);
